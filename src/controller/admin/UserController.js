...
export const createUser = async (req, res) => {
  try {
    const { fullName, email, phoneNumber, password, address, gender, schoolId, schoolName, schoolEmail } = req.body;

    // Multi-tenant: If creator is a tenant admin, lock user to same school
    const assignedSchoolId = req.admin?.schoolId || schoolId;

    const existingUser = await User.findOne({ email, isDeleted: false, schoolId: assignedSchoolId });
    if (existingUser) {
      return ResponseHandler(res, StatusCodes.CONFLICT, 'User with this email already exists');
    }
    ...
    // Send email logic
    sendSubscriptionBaseMail(`Your User Account Registration OTP is: ${otp}. Please verify to activate your account.`, [email])
      .catch(err => console.error(`Error sending User Registration OTP: ${err}`));

    return ResponseHandler(res, StatusCodes.CREATED, 'User created successfully. OTP sent for verification.', {
      userId: newUser._id,
      email: newUser.email
    });
  } catch (error) {
...
export const verifyUserOtp = async (req, res) => {
  try {
    ...
    if (!user) return ResponseHandler(res, StatusCodes.NOT_FOUND, responseMessage.USER_NOT_FOUND);
    ...
    if (req.admin?.schoolId && req.admin.schoolId.toString() !== user.schoolId?.toString()) {
      return ResponseHandler(res, StatusCodes.FORBIDDEN, 'Access denied for this user');
    }
    ...
    return ResponseHandler(res, StatusCodes.OK, 'User verified and activated successfully.');
  } catch (error) {
    logger.error(error);
    return CatchErrorHandler(res, error);
  }
...
export const getAllUser = async (req, res) => {
  try {
    ...
    return ResponseHandler(res, StatusCodes.OK, responseMessage.USER_FETCH_SUCCESS, {
      totalArrayLength,
      pageNumber: page,
      perPageData: limit,
      data: users,
...
export const updateUser = async (req, res) => {
  try {
    ...
    return ResponseHandler(res, StatusCodes.OK, 'User updated successfully', updatedUser);
  } catch (error) {
    logger.error(error);
    return CatchErrorHandler(res, error);
  }
...
export const deleteUser = async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate(query, { isDeleted: true }, { new: true });

    if (!user) {
      return ResponseHandler(res, StatusCodes.NOT_FOUND, responseMessage.USER_NOT_FOUND);
    }

    return ResponseHandler(res, StatusCodes.OK, responseMessage.USER_DELETE_SUCCESS, null);
  } catch (error) {
    logger.error(error);
    return CatchErrorHandler(res, error);
  }
...
export const userActionStatus = async (req, res) => {
  try {
    ...
    return ResponseHandler(res, StatusCodes.OK, responseMessage.USER_STATUS_UPDATED, updatedUser);
  } catch (error) {
    logger.error(error);
    return CatchErrorHandler(res, error);
  }
